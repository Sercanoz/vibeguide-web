...
export default function UsersPage() {
  ...
  async function loadUsers(search: string, role: RoleFilter) {
    ...
    if (res.ok) {
      ...
      setError(res.error ?? "Kullanıcılar yüklenemedi");
    }
    setLoading(false);
  }
  ...
  async function changeRole(id: number, role: "Tourist" | "Guide" | "Admin") {
    if (!window.confirm(`Bu kullanıcının rolünü "${role}" olarak değiştirmek istiyor musunuz?`)) return;
    setActing(id);
    const res = await adminApi.changeUserRole(id, role);
    if (res.ok) {
      setUsers((prev) => prev.map((u) => u.id === id ? { ...u, role } : u));
      showFlash("Rol güncellendi.", "ok");
    } else {
      showFlash(res.error ?? "Rol değiştirilemedi", "err");
    }
    setActing(null);
  }

  async function deleteUser(id: number, name: string) {
    if (!window.confirm(`"${name}" kullanıcısını silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`)) return;
    setActing(id);
    const res = await adminApi.deleteUser(id);
    if (res.ok) {
      setUsers((prev) => prev.filter((u) => u.id !== id));
      showFlash("Kullanıcı silindi.", "ok");
    } else {
      showFlash(res.error ?? "Kullanıcı silinemedi", "err");
    }
    setActing(null);
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-5 sm:px-8 py-10">
      <h1 className="text-2xl font-black tracking-tight text-vg-ink">Kullanıcı Yönetimi</h1>
      <p className="mt-1 text-sm text-vg-muted">Kullanıcıları ara, rollerini değiştir veya sil.</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-vg-border bg-vg-bg-soft">
                <th className="px-4 py-3 text-left text-xs font-bold text-vg-muted uppercase tracking-wide">Ad</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-vg-muted uppercase tracking-wide hidden sm:table-cell">E-posta</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-vg-muted uppercase tracking-wide">Rol</th>
                <th className="px-4 py-3 text-left text-xs font-bold text-vg-muted uppercase tracking-wide hidden md:table-cell">Kayıt</th>
                <th className="px-4 py-3 text-right text-xs font-bold text-vg-muted uppercase tracking-wide">İşlemler</th>
              </tr>
            </thead>
