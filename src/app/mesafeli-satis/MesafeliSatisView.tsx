"use client";

import LegalMarkdown from "@/components/LegalMarkdown";
import { useT } from "@/components/LanguageProvider";

const SUBTITLE_TR =
  "6502 Sayılı Tüketicinin Korunması Hakkında Kanun · Son Güncelleme: 04.06.2026";
const SUBTITLE_EN =
  "Under Law No. 6502 on Consumer Protection · Last updated: 04.06.2026";

const BODY_TR = `
**6502 Sayılı Tüketicinin Korunması Hakkında Kanun ve Mesafeli Sözleşmeler Yönetmeliği Kapsamında**

## MADDE 1 – TARAFLAR

### SATICI

Unvan: **VİBECORE TURİZM SEYAHAT ACENTASI VE DİJİTAL HİZMETLER TİCARET LİMİTED ŞİRKETİ**

Adres: Beldibi Mah. Belmar Sk. No:9/1 İç Kapı No:4, Marmaris / Muğla / Türkiye

Vergi Dairesi: Marmaris Vergi Dairesi

Vergi No: 9251328389

Ticaret Sicil No: 12686

E-posta: [support@vibeguideapp.com](mailto:support@vibeguideapp.com)

Telefon: +90 530 828 76 96

Bundan sonra "SATICI" veya "VibeGuide" olarak anılacaktır. VİBECORE, VibeGuide platformunun sahibi ve işletmecisi (aracı hizmet sağlayıcı) konumundadır.

### SEYAHAT ACENTESİ (HİZMET SAĞLAYICI)

VibeGuide platformu üzerinden sunulan tur, rezervasyon ve seyahat hizmetleri; 1618 sayılı Seyahat Acentaları ve Seyahat Acentaları Birliği Kanunu uyarınca aşağıda bilgileri verilen TÜRSAB A Grubu seyahat acentesi tarafından organize edilmekte ve yürütülmektedir:

Unvan: **ANADOLU TUR. SEY. TUR. TİC. LTD. ŞTİ.**

Acenta Ticari Unvanı: **Helliantus Anadolu Tours**

TÜRSAB Belge No: **5058**

Adres: Cumhuriyet Mah., İnönü Bulvarı, Şeref Gedik İş Merkezi, Kuşadası / Aydın / Türkiye

Seyahat hizmetinin ifası, yukarıda belirtilen TÜRSAB belgeli seyahat acentesi tarafından gerçekleştirilir.

### ALICI

VibeGuide mobil uygulaması, internet sitesi veya diğer dijital kanalları üzerinden rezervasyon gerçekleştiren gerçek veya tüzel kişi kullanıcıdır.

ALICI'nın rezervasyon sırasında beyan ettiği ad, soyad, iletişim ve ödeme bilgileri sözleşmenin ayrılmaz parçasıdır.

## MADDE 2 – SÖZLEŞMENİN KONUSU

İşbu sözleşmenin konusu, ALICI'nın VibeGuide platformu üzerinden elektronik ortamda satın aldığı turizm, rehberlik, kültürel deneyim, gezi ve benzeri hizmetlerin satışı ve ifası ile ilgili olarak tarafların hak ve yükümlülüklerinin belirlenmesidir.

İşbu sözleşme 6502 Sayılı Tüketicinin Korunması Hakkında Kanun ve Mesafeli Sözleşmeler Yönetmeliği hükümlerine uygun olarak düzenlenmiştir.

## MADDE 3 – PLATFORMUN ROLÜ

VibeGuide, kullanıcılar ile doğrulanmış hizmet sağlayıcılar arasında rezervasyon yapılmasını sağlayan dijital bir platformdur.

Platform üzerinden sunulan hizmetler;

* doğrudan VibeGuide tarafından,
* lisanslı turist rehberleri tarafından,
* TÜRSAB üyesi seyahat acenteleri tarafından,
* veya VibeGuide'ın iş ortakları tarafından

sunulabilir.

Hizmeti fiilen sağlayan taraf rezervasyon ekranında açıkça belirtilir.

## MADDE 4 – HİZMETİN TEMEL NİTELİKLERİ

Satın alınan hizmete ilişkin;

* hizmet türü,
* tarih,
* saat,
* süre,
* buluşma noktası,
* rehber dili,
* katılımcı sayısı,
* fiyat,
* dahil olan hizmetler,
* dahil olmayan hizmetler,
* özel koşullar

rezervasyon tamamlanmadan önce ALICI'ya sunulur.

ALICI rezervasyonu tamamlayarak bu bilgileri inceleyip kabul ettiğini beyan eder.

Sipariş özeti ve rezervasyon detayları işbu sözleşmenin ayrılmaz parçasıdır.

## MADDE 5 – TOPLAM BEDEL VE ÖDEME

Hizmet bedeli tüm vergiler dahil olarak rezervasyon ekranında gösterilir.

Aksi belirtilmedikçe;

* müze giriş ücretleri,
* ören yeri giriş ücretleri,
* ulaşım giderleri,
* yemek ve içecekler,
* kişisel harcamalar

fiyata dahil değildir.

Ödemeler lisanslı ödeme kuruluşları aracılığıyla güvenli şekilde gerçekleştirilir.

Ödeme kartı bilgileri VibeGuide sistemlerinde saklanmaz.

## MADDE 6 – SÖZLEŞMENİN KURULMASI

ALICI'nın Ön Bilgilendirme Formu'nu ve işbu Mesafeli Satış Sözleşmesi'ni elektronik ortamda onaylayarak ödemeyi tamamlaması ile sözleşme kurulmuş sayılır.

Rezervasyon onayı elektronik ortamda ALICI'ya gönderilir.

## MADDE 7 – HİZMETİN SUNULMASI

Hizmet, rezervasyon sırasında belirtilen tarih, saat ve buluşma noktasında sunulur.

ALICI belirtilen tarih ve saatte hazır bulunmakla yükümlüdür.

## MADDE 8 – ALICININ YÜKÜMLÜLÜKLERİ

ALICI;

* doğru ve güncel bilgi vermeyi,
* rezervasyon saatinde hazır bulunmayı,
* deneyim kurallarına uymayı,
* diğer katılımcılara saygılı davranmayı,
* fiziksel koşullarını ve sağlık durumunu göz önünde bulundurarak katılım sağlamayı

kabul eder.

## MADDE 9 – DAVRANIŞ VE GÜVENLİK KURALLARI

Hizmet sağlayıcı veya VibeGuide;

* diğer katılımcıların güvenliğini tehlikeye atan,
* saldırgan davranışlarda bulunan,
* yasa dışı faaliyetlerde bulunan,
* deneyimin yürütülmesini ciddi şekilde engelleyen

kişileri deneyimden çıkarma hakkını saklı tutar.

Bu durumda ücret iadesi yapılmayabilir.

## MADDE 10 – KATILIM SAĞLANMAMASI (NO-SHOW)

ALICI'nın belirtilen tarih ve saatte buluşma noktasında bulunmaması veya makul bekleme süresi içerisinde ulaşılamaması halinde rezervasyon kullanılmamış sayılır.

Bu durumda ücret iadesi yapılmayabilir.

## MADDE 11 – REHBERİN VEYA HİZMET SAĞLAYICININ KATILAMAMASI

Hizmet sağlayıcının rezervasyonu gerçekleştirememesi halinde VibeGuide;

* alternatif rehber,
* alternatif tarih,
* eşdeğer deneyim,
* veya tam iade

seçeneklerinden birini sunabilir.

## MADDE 12 – GRUP DENEYİMLERİ VE MİNİMUM KATILIM

Bazı grup deneyimleri belirli bir minimum katılımcı sayısına ulaşılması şartına bağlı olabilir.

Minimum katılım sayısına ulaşılamaması halinde VibeGuide;

* alternatif tarih,
* alternatif deneyim,
* veya tam iade

sunabilir.

## MADDE 13 – MÜCBİR SEBEPLER

Doğal afetler, savaş, terör olayları, salgın hastalıklar, resmi makam kararları, ulaşım kesintileri, güvenlik riskleri veya benzeri mücbir sebepler nedeniyle hizmetin gerçekleştirilememesi halinde taraflar sorumlu tutulamaz.

Bu durumda program değiştirilebilir, ertelenebilir veya iptal edilebilir.

## MADDE 14 – ÜÇÜNCÜ TARAF HİZMETLER

Müzeler, ören yerleri, ulaşım şirketleri, restoranlar ve diğer üçüncü taraf hizmet sağlayıcıların faaliyetlerinden kaynaklanan değişiklikler veya aksaklıklar VibeGuide'ın doğrudan kontrolü dışında olabilir.

## MADDE 15 – CAYMA HAKKI

6502 Sayılı Kanun ve Mesafeli Sözleşmeler Yönetmeliği'nin 15. maddesi uyarınca belirli bir tarihte veya dönemde yerine getirilecek turizm ve boş zaman değerlendirme hizmetlerinde cayma hakkı bulunmamaktadır.

ALICI bu durumu kabul eder.

## MADDE 16 – İPTAL VE İADE

İptal ve iade koşulları rezervasyon sırasında belirtilen İptal ve İade Politikası'na tabidir.

Aksi belirtilmedikçe;

* deneyim başlangıcından en az 24 saat önce yapılan iptallerde tam iade,
* 24 saatten daha kısa süre kala yapılan iptallerde iade yapılmaması,
* hizmet sağlayıcı kaynaklı iptallerde tam iade,
* mücbir sebep durumlarında olay bazında değerlendirme

esas alınır.

## MADDE 17 – SORUMLULUĞUN SINIRLANDIRILMASI

VibeGuide, yürürlükteki mevzuatın izin verdiği ölçüde;

* kullanıcıların kişisel eşyalarının kaybından,
* kullanıcı kaynaklı kazalardan,
* kullanıcıların kendi kusur veya ihmallerinden doğan zararlardan,
* üçüncü taraf hizmet sağlayıcılardan kaynaklanan aksaklıklardan

sorumlu değildir.

Bu madde tüketici mevzuatından kaynaklanan zorunlu hakları ortadan kaldırmaz.

## MADDE 18 – KİŞİSEL VERİLER

ALICI'nın kişisel verileri, 6698 Sayılı Kişisel Verilerin Korunması Kanunu (KVKK), ilgili mevzuat ve VibeGuide Gizlilik Politikası hükümlerine uygun olarak işlenir.

## MADDE 19 – UYUŞMAZLIKLARIN ÇÖZÜMÜ

İşbu sözleşmeden doğan uyuşmazlıklarda Türk Hukuku uygulanır.

Ticaret Bakanlığı tarafından her yıl ilan edilen parasal sınırlar dahilinde Tüketici Hakem Heyetleri ve Tüketici Mahkemeleri yetkilidir.

Başvurular e-Devlet üzerinden TÜBİS sistemi aracılığıyla da yapılabilir.

## MADDE 20 – KISMİ GEÇERSİZLİK

İşbu sözleşmenin herhangi bir hükmünün geçersiz veya uygulanamaz hale gelmesi, diğer hükümlerin geçerliliğini etkilemez.

## MADDE 21 – YÜRÜRLÜK

ALICI, rezervasyonu tamamlayarak işbu Mesafeli Satış Sözleşmesi'ni, Ön Bilgilendirme Formu'nu ve ilgili politikaları elektronik ortamda okuduğunu, anladığını ve kabul ettiğini beyan eder.

İşbu sözleşme rezervasyonun tamamlandığı tarihte yürürlüğe girer.

---

İlgili Belgeler: [Ön Bilgilendirme Formu](/on-bilgilendirme) · [İptal ve İade Politikası](/cancellation-policy) · [Gizlilik Politikası](/privacy) · [KVKK Aydınlatma Metni](/kvkk)
`;

const BODY_EN = `
**Under Law No. 6502 on Consumer Protection and the Regulation on Distance Contracts**

## ARTICLE 1 – PARTIES

### SELLER

Title: **VİBECORE TURİZM SEYAHAT ACENTASI VE DİJİTAL HİZMETLER TİCARET LİMİTED ŞİRKETİ**

Address: Beldibi Mah. Belmar Sk. No:9/1 İç Kapı No:4, Marmaris / Muğla / Türkiye

Tax Office: Marmaris Tax Office

Tax No: 9251328389

Trade Registry No: 12686

Email: [support@vibeguideapp.com](mailto:support@vibeguideapp.com)

Phone: +90 530 828 76 96

Hereinafter referred to as the "SELLER" or "VibeGuide". VİBECORE is the owner and operator of the VibeGuide platform (intermediary service provider).

### TRAVEL AGENCY (SERVICE PROVIDER)

Tours, bookings and travel services offered through the VibeGuide platform are organized and operated by the following TÜRSAB A Group travel agency, in accordance with Turkish Law No. 1618 on Travel Agencies:

Title: **ANADOLU TUR. SEY. TUR. TİC. LTD. ŞTİ.**

Agency Trade Name: **Helliantus Anadolu Tours**

TÜRSAB License No: **5058**

Address: Cumhuriyet Mah., İnönü Bulvarı, Şeref Gedik İş Merkezi, Kuşadası / Aydın / Türkiye

The travel service is performed by the TÜRSAB-licensed travel agency stated above.

### BUYER

The natural or legal person who makes a booking through the VibeGuide mobile app, website, or other digital channels.

The name, surname, contact, and payment details declared by the BUYER during booking are an integral part of this agreement.

## ARTICLE 2 – SUBJECT OF THE AGREEMENT

The subject of this agreement is to define the rights and obligations of the parties regarding the sale and provision of tourism, guiding, cultural-experience, tour, and similar services purchased electronically by the BUYER through the VibeGuide platform.

This agreement has been prepared in accordance with Law No. 6502 on Consumer Protection and the Regulation on Distance Contracts.

## ARTICLE 3 – ROLE OF THE PLATFORM

VibeGuide is a digital platform that enables bookings between users and verified service providers.

Services offered through the platform may be provided:

* directly by VibeGuide,
* by licensed tourist guides,
* by travel agencies that are members of TÜRSAB,
* or by VibeGuide's business partners.

The party actually providing the service is clearly indicated on the booking screen.

## ARTICLE 4 – ESSENTIAL CHARACTERISTICS OF THE SERVICE

For the purchased service, the following are presented to the BUYER before the booking is completed:

* service type,
* date,
* time,
* duration,
* meeting point,
* guide language,
* number of participants,
* price,
* included services,
* excluded services,
* special conditions.

By completing the booking, the BUYER declares that they have reviewed and accepted this information.

The order summary and booking details are an integral part of this agreement.

## ARTICLE 5 – TOTAL PRICE AND PAYMENT

The service price is shown on the booking screen including all taxes.

Unless stated otherwise, the following are not included in the price:

* museum entrance fees,
* heritage-site entrance fees,
* transport costs,
* food and beverages,
* personal expenses.

Payments are processed securely through licensed payment institutions.

Payment card details are not stored on VibeGuide's systems.

## ARTICLE 6 – FORMATION OF THE AGREEMENT

The agreement is deemed formed once the BUYER electronically approves the Pre-Information Form and this Distance Sales Agreement and completes payment.

A booking confirmation is sent to the BUYER electronically.

## ARTICLE 7 – PROVISION OF THE SERVICE

The service is provided at the date, time, and meeting point specified during booking.

The BUYER is obliged to be present at the specified date and time.

## ARTICLE 8 – OBLIGATIONS OF THE BUYER

The BUYER agrees to:

* provide accurate and up-to-date information,
* be present at the booking time,
* comply with the experience rules,
* behave respectfully toward other participants,
* participate taking their physical condition and health into account.

## ARTICLE 9 – CONDUCT AND SAFETY RULES

The service provider or VibeGuide reserves the right to remove from the experience anyone who:

* endangers the safety of other participants,
* engages in aggressive behaviour,
* engages in illegal activities,
* seriously obstructs the conduct of the experience.

In such cases, a refund may not be issued.

## ARTICLE 10 – NON-ATTENDANCE (NO-SHOW)

If the BUYER is not present at the meeting point at the specified date and time, or cannot be reached within a reasonable waiting period, the booking is deemed unused.

In such cases, a refund may not be issued.

## ARTICLE 11 – NON-ATTENDANCE OF THE GUIDE OR SERVICE PROVIDER

If the service provider is unable to carry out the booking, VibeGuide may offer one of the following:

* alternative guide,
* alternative date,
* equivalent experience,
* or a full refund.

## ARTICLE 12 – GROUP EXPERIENCES AND MINIMUM PARTICIPATION

Some group experiences may be subject to reaching a minimum number of participants.

If the minimum participation count is not reached, VibeGuide may offer:

* alternative date,
* alternative experience,
* or a full refund.

## ARTICLE 13 – FORCE MAJEURE

The parties cannot be held liable where the service cannot be performed due to natural disasters, war, acts of terrorism, epidemics, decisions of official authorities, transport disruptions, security risks, or similar force-majeure events.

In such cases, the programme may be modified, postponed, or cancelled.

## ARTICLE 14 – THIRD-PARTY SERVICES

Changes or disruptions arising from the activities of museums, heritage sites, transport companies, restaurants, and other third-party service providers may be outside VibeGuide's direct control.

## ARTICLE 15 – RIGHT OF WITHDRAWAL

Pursuant to Article 15 of Law No. 6502 and the Regulation on Distance Contracts, there is no right of withdrawal for tourism and leisure services to be performed on a specific date or period.

The BUYER accepts this.

## ARTICLE 16 – CANCELLATION AND REFUND

Cancellation and refund conditions are subject to the Cancellation & Refund Policy indicated during booking.

Unless stated otherwise, the following apply:

* full refund for cancellations made at least 24 hours before the experience starts,
* no refund for cancellations made less than 24 hours before,
* full refund for cancellations caused by the service provider,
* case-by-case assessment in force-majeure situations.

## ARTICLE 17 – LIMITATION OF LIABILITY

To the extent permitted by applicable law, VibeGuide is not liable for:

* loss of users' personal belongings,
* accidents caused by users,
* damages arising from users' own fault or negligence,
* disruptions caused by third-party service providers.

This article does not remove mandatory rights arising from consumer legislation.

## ARTICLE 18 – PERSONAL DATA

The BUYER's personal data is processed in accordance with Law No. 6698 on the Protection of Personal Data (KVKK), the relevant legislation, and VibeGuide's Privacy Policy.

## ARTICLE 19 – DISPUTE RESOLUTION

Turkish law applies to disputes arising from this agreement.

Within the monetary limits announced annually by the Ministry of Trade, Consumer Arbitration Committees and Consumer Courts have jurisdiction.

Applications may also be made through the TÜBİS system via e-Devlet.

## ARTICLE 20 – PARTIAL INVALIDITY

If any provision of this agreement becomes invalid or unenforceable, it does not affect the validity of the other provisions.

## ARTICLE 21 – ENTRY INTO FORCE

By completing the booking, the BUYER declares that they have electronically read, understood, and accepted this Distance Sales Agreement, the Pre-Information Form, and the related policies.

This agreement enters into force on the date the booking is completed.

---

Related Documents: [Pre-Information Form](/on-bilgilendirme) · [Cancellation & Refund Policy](/cancellation-policy) · [Privacy Policy](/privacy) · [Data Protection Notice](/kvkk)
`;

export default function MesafeliSatisView() {
  const { locale } = useT();
  const tr = locale === "tr";
  return (
    <LegalMarkdown
      title={tr ? "Mesafeli Satış Sözleşmesi" : "Distance Sales Agreement"}
      subtitle={tr ? SUBTITLE_TR : SUBTITLE_EN}
      body={tr ? BODY_TR : BODY_EN}
    />
  );
}
