    ...
    <nav className="sticky top-0 left-0 right-0 z-50 bg-white backdrop-blur-xl border-b border-black/[0.06] shadow-sm shadow-black/5">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 h-16">
        ...
        <div className="hidden gap-8 text-sm font-medium md:flex">
          {navLinks.map((l) => (
            <a key={l.href} href={l.href} className="text-neutral-700 hover:text-black transition-colors">{l.label}</a>
          ))}
        </div>
        ...
        <div className="px-6 pb-5 pt-2 border-t border-black/5 space-y-1">
          {navLinks.map((l) => (
            <a
              ...
              className="flex items-center py-3 text-sm font-semibold text-neutral-800 hover:text-black border-b border-black/5 last:border-0 transition-colors"
            >
              {l.label}
            </a>
          ))}
